...
func (r *PayloadRepository) FormatActionMessage(payload entities.ActionEventPayload) interface{} {
    return map[string]interface{}{
        "embeds": []map[string]interface{}{
            {
                "title":       fmt.Sprintf("Action: %s", payload.ID),
                "description": fmt.Sprintf("Status: %s", payload.Status),
                "color":       3447003,
            },
        },
    }
